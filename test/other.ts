...
describe("OtherERC20TokenFactory", function () {
  ...
  beforeEach(async function () {
    [owner, addr1, addr2, addr3, feesReceiver, newFeesReceiver] = await ethers.getSigners();

    OtherERC20TokenFactory = await ethers.getContractFactory("OtherERC20TokenFactory");
    ...
    BurnablePausableERC20Token = await ethers.getContractFactory("BurnablePausableERC20Token");
  });
  ...
  it("Should create a pausable ERC20 token", async function () {
    ...
    await expect(token.connect(addr1).transfer(owner.address, ethers.parseEther("1000"))).to.be.revertedWith("Pausable: paused");
    await token.connect(addr1).unpause()
    await token.connect(addr1).transfer(owner.address, ethers.parseEther("1200"))
  ...
  it("Should create a burnable and pausable ERC20 token", async function () {
    ...
    await token.pause()
    await expect(token.burn( ethers.parseEther("1000"))).to.be.revertedWith("Pausable: paused");
    await token.unpause()
